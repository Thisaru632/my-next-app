...
const driverAgreementSections: AgreementSection[] = [
  {
    id: 'section-1',
    number: '01',
...
export default function PrivacyAndPoliciesTabbedPage() {
  const params = useParams();
  const router = useRouter();

  // Extract and decode the tab segment from the URL
  const rawTab = params?.tab ? (Array.isArray(params.tab) ? params.tab[0] : params.tab) : '';
  const decodedTab = decodeURIComponent(rawTab).toLowerCase();

  // Determine active tab based on URL subpath
  const isCustomer =
    decodedTab === 'customer-terms' ||
    decodedTab === 'customer terms' ||
    decodedTab === 'customer';

  const activeTab: 'driver' | 'customer' = isCustomer ? 'customer' : 'driver';

  const [isAgreed, setIsAgreed] = useState(false);

  const handleTabChange = (tab: 'driver' | 'customer') => {
    const slug = tab === 'driver' ? 'driver-terms' : 'customer-terms';
    router.push(`/privacy-and-policies/${slug}`, { scroll: false });
  };
  ...
  return (
    <main className="min-h-screen bg-[#faf8f5] pt-28 md:pt-32 pb-20 text-gray-800 font-sans">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#071d24] via-[#0d313d] to-[#0a2540] text-white p-6 sm:p-10 md:p-12 shadow-xl shadow-gray-900/10 border border-emerald-900/30">
          ...
          <div className="relative z-10">
            ...
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-black tracking-tight leading-snug mb-3">
              Privacy &amp; <span className="text-green-400">Policies</span>
            </h1>

            <p className="text-gray-300 text-xs sm:text-sm md:text-base leading-relaxed max-w-3xl">
              Senu Cabs &amp; Tours ආයතනයේ රියදුරු හවුල්කරුවන් සහ පාරිභෝගිකයින් සඳහා වන නිල ප්‍රතිපත්ති සහ නියමාවලීන්.
            </p>
          </div>
        </div>
      </section>

      {/* Tabs Switcher: Driver Terms vs Customer Terms */}
      <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
        <div className="flex items-center gap-3 p-1.5 bg-gray-200/70 backdrop-blur-sm rounded-2xl w-fit border border-gray-300/60 shadow-inner">
          <button
      ...
      {/* TAB CONTENT: DRIVER TERMS */}
      {activeTab === 'driver' && (
        <>
          {/* Driver Title Sub-card */}
          <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mb-6">
            <div className="bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-8 border border-gray-200/80 shadow-sm">
              <h2 className="text-xl sm:text-2xl font-black text-gray-900 mb-2">
                රියදුරු ලියාපදිංචි වීමේ <span className="text-green-600">පරිශීලක ගිවිසුම</span>
              </h2>
              <div className="text-xs sm:text-sm font-bold text-green-700 mb-4">
                අවසන් පිළිගැනීම
              </div>
              <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100 text-xs sm:text-sm text-gray-700 leading-relaxed space-y-2">
                <p>
          ...
          {/* Mobile-Only Table of Contents Section (පටුන) - Displayed at top on mobile (<lg) */}
          <section className="block lg:hidden max-w-6xl mx-auto px-4 sm:px-6 mb-8 print:hidden">
          ...
          <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="lg:col-span-8 space-y-6">
                {driverAgreementSections.map((section) => (
                  <article
                    key={section.id}
                    id={section.id}
              ...
              {/* Quick Table of Contents Sidebar (4 cols) - Preserved on desktop view */}
      ...
      {/* TAB CONTENT: CUSTOMER TERMS (Awaiting user's content) */}
      {activeTab === 'customer' && (
        <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white rounded-2xl sm:rounded-3xl p-8 sm:p-12 border border-gray-200/80 shadow-sm text-center">
            <div className="w-14 h-14 rounded-2xl bg-green-50 border border-green-200 flex items-center justify-center text-green-600 mx-auto mb-5 shadow-sm">
              <Clock size={28} />
            </div>
            ...
            <p className="text-sm sm:text-base text-gray-600 max-w-lg mx-auto leading-relaxed">
              පාරිභෝගික නියමයන් සහ කොන්දේසි නුදුරේදීම මෙහි ප්‍රකාශයට පත් කරනු ලැබේ.
            </p>
            <div className="mt-6 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gray-100 text-gray-600 text-xs font-semibold">
